...
object Main_local {
  def main(args: Array[String]): Unit = {
    ...
    val input = sc.textFile("news.2013.en.short").map(line => line.split(" ").toSeq)
    ...
    val skipgram = new SkipGram().setNumPartitions(8).setNumIterations(1000).setNegative(5).setMinCount(5).setWindow(5).setVectorSize(100).setSample(0.001)
    val model = skipgram.fit(words)
    //val synonyms = model.findSynonyms(args(4), 10)
    val synonyms = model.findSynonyms("apple", 10)
    ...
    //skipgram.cleanSyn()
/*
    val msskipgram = new MSSkipGram(skipgram).setNumPartitions(1).setNumIterations(20).setNegative(1).setNumSenses(2).setMinCount(10).setWindow(10).setVectorSize(500).setSample(0.1)
    ...
    for (i <- 0 to 1) {
      val newSynonyms = newModel.findSynonyms("apple"+i, 10)
